          ...
          {filteredExercises.map((ex) => {
            ...
            return (
              <div key={ex.id} className="transition-colors">
                <div className="p-3.5 flex items-center justify-between hover:bg-slate-50/70 dark:hover:bg-slate-800/40 transition-colors group">
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {exSets.length > 0 && (
                          <button
                            ...
                            className="text-[9px] bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-1.5 py-0.2 rounded font-semibold hover:bg-slate-200 dark:hover:bg-slate-700 flex items-center gap-0.5"
                            title="Ver historial de series de este ejercicio"
                          >
                            <span>{exSets.length} series</span>
                ...
                {/* Expanded series history for this specific exercise */}
                {isExpanded && exSets.length > 0 && (
                  <div className="bg-slate-50/70 dark:bg-slate-800/60 p-3 border-t border-slate-100 dark:border-slate-800 animate-in slide-in-from-top-1 duration-200">
                    <h4 className="text-[11px] font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                      <Calendar className="w-3 h-3 text-[#0e7490] dark:text-cyan-400" />
                      Historial de series ingresadas ({ex.name})
                    </h4>
                    <div className="space-y-1.5">
                      {exSets.map((s) => (
                        <div
                          key={s.id}
                          className="bg-white dark:bg-slate-800 p-2 rounded-lg border border-slate-200/60 dark:border-slate-700/60 flex items-center justify-between text-xs"
                        >
                          ...
                          <div className="flex items-center gap-1">
                            {onDuplicateSet && (
                              <button
                                ...
                                className="text-slate-400 hover:text-[#0e7490] dark:hover:text-cyan-400 p-1"
                                title="Repetir serie"
                              >
